...
namespace fre {
    ...
    double PathDepOption::PriceByMC(const MCModel& Model, long N, double epsilon)
    {
        ...
        SamplePath S(m);
        for (long i = 0; i < N; i++) //long int
        {
            Model.GenerateSamplePath(T, m, S);
            H = (i * H + Payoff(S)) / (i + 1.0); //和求和等价 (注意下一轮循环时i变成i+1 分母被抵消)
            Hsq = (i * Hsq + pow(Payoff(S), 2.0)) / (i + 1.0); //和求和等价 (注意下一轮循环时i变成i+1 分母被抵消)
            Rescale(S, 1.0 + epsilon);
            Heps = (i * Heps + Payoff(S)) / (i + 1.0);
            Rescale(S, (1.0 - epsilon)/(1.0 + epsilon)); //注意rescale回去
        ...
        gamma= exp(-Model.GetR() * T)*(Heps-2*H+Hgamma)/(Model.GetS0()*epsilon);
        return Price;
    }
